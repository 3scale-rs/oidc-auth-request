...
build: ## Build WASM filter
	...
	ln -sf ../../target/$(TARGET)/$(BUILD)/oidc_auth_request.wasm $(PROJECT_PATH)/compose/wasm/
	ln -f $(PROJECT_PATH)/target/$(TARGET)/$(BUILD)/oidc_auth_request.wasm $(PROJECT_PATH)/servicemesh/

clean: ## Clean WASM filter
	cargo clean
	rm -f $(PROJECT_PATH)/compose/wasm/oidc_auth_request.wasm
	rm -f $(PROJECT_PATH)/servicemesh/oidc_auth_request.wasm
